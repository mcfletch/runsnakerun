...
class TestSpeedScope(unittest.TestCase):
    ...
    def test_load_simple(self):
        ssl = speedscopeloader.SpeedScopeFile(
            filename=self.SAMPLED_FILE,
        )
        ...
        for root in profile.roots:
            ...
            assert not root.local,root.frame.index # no samples in fixture has time sampled directly in top call
        assert profile.total
        assert sum([root.cumulative for root in profile.roots],0) == len([sample for sample in profile.samples if sample])
