...
def records_as_index(records):
    index = dict([(x['address'], x) for x in records])
    shared = {}
...
class MeliaeTests(unittest.TestCase):
    ...
    def test_group_children(self):
        ...
        strings = [
            {'size': 1, 'type': 'str', 'address': i + 12, 'refs': []} for i in range(10)
        ]
        records = [
            {
                'size': 10,
                'type': 'moo',
                'address': 1,
                'refs': [s['address'] for s in strings],
            },
            {'size': 1, 'type': 'dict', 'address': 2, 'refs': [1]},
            {'size': 1, 'type': 'module', 'address': 3, 'refs': [2]},
        ] + strings
    ...
    def test_recursive_shared(self):
        ...
        records = [
            ...
            {'size': 1, 'type': 'module', 'address': 3, 'refs': [2]},
            {
                ...
                'address': 5,
                'refs': [4, 1],
            },  # should become a single ref to loop...
        ...
        for module in [3, 6]:
            meliaeloader.recurse_module(
                index[module], index=index, shared=shared,
            )
